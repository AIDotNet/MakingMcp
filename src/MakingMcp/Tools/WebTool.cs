...
public class WebTool
{
    ...
    private static string ConvertToMarkdown(string htmlOrText)
    {
        ...
        var withoutScripts =
            Regex.Replace(htmlOrText, "<script[\\s\\S]*?</script>", string.Empty, RegexOptions.IgnoreCase);
        var withoutStyles = Regex.Replace(withoutScripts, "<style[\\s\\S]*?</style>", string.Empty,
            RegexOptions.IgnoreCase);
        var text = Regex.Replace(withoutStyles, "<[^>]+>", string.Empty);
        text = Regex.Replace(text, "&nbsp;", " ", RegexOptions.IgnoreCase);
        text = Regex.Replace(text, "&amp;", "&", RegexOptions.IgnoreCase);
        text = Regex.Replace(text, "&lt;", "<", RegexOptions.IgnoreCase);
        text = Regex.Replace(text, "&gt;", ">", RegexOptions.IgnoreCase);
        return Regex.Replace(text, "\\s+", match => match.Value.Contains('\n') ? "\n" : " ").Trim();
    }

    private static string BuildFocusedSummary(string markdown, string prompt, int maxParagraphs)
    {
        if (string.IsNullOrWhiteSpace(markdown) || string.IsNullOrWhiteSpace(prompt))
        {
            return string.Empty;
        }
        ...
        var scored = paragraphs
            ...
            {
                Paragraph = paragraph,
                Score = keywords.Count(keyword => paragraph.Contains(keyword, StringComparison.OrdinalIgnoreCase)),
            })
