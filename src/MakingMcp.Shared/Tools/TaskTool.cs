...
public class TaskTool
{
    ...
    string prompt)
    {
        ...
        try
        {
            ...
            {
                Log.Warning("[Task {TaskId}] Task failed: description is empty", taskId);
                return Error("description must be provided.");
            }

            if (string.IsNullOrWhiteSpace(prompt))
            {
                Log.Warning("[Task {TaskId}] Task failed: prompt is empty", taskId);
            ...
            {
                Log.Information("[Task {TaskId}] Task completed with result length: {Length}", taskId, result?.Length ?? 0);
            ...
            {
                Log.Information("[Task {TaskId}] Starting agent execution", taskId);
                ...
                Log.Information("[Task {TaskId}] Agent execution completed, total output length: {Length}", taskId, sb.Length);
            });

            var timeoutTask = Task.Delay(TimeSpan.FromMinutes(10));
            var completedTask = await Task.WhenAny(completionSource.Task, agentTask, timeoutTask);

            if (completedTask == timeoutTask)
            {
                Log.Warning("[Task {TaskId}] Task execution timed out after 10 minutes", taskId);
                return Error("Agent execution timed out after 10 minutes.");
            }

            if (completionResult != null)
            {
                Log.Information("[Task {TaskId}] Task completed successfully via completion callback", taskId);
            ...
            Log.Information("[Task {TaskId}] Task completed successfully, result length: {Length}", taskId, result.Length);
    ...
    {
        Log.Debug("[Task {TaskId}] Creating kernel for agent", taskId);
        ...
        kernelBuilder.AddOpenAIChatCompletion(OpenAIOptions.TASK_MODEL, new Uri(OpenAIOptions.OPENAI_ENDPOINT),
            OpenAIOptions.API_KEY, httpClient: httpClient);
        ...
        {
            Log.Information("[Task {TaskId}] Invoking function: {PluginName}.{FunctionName}", taskId, e.Function.PluginName, e.Function.Name);
            if (e.Arguments != null && e.Arguments.Count > 0)
            {
                foreach (var arg in e.Arguments)
                {
                    var value = arg.Value?.ToString();
                    var truncated = value != null && value.Length > 200 ? value.Substring(0, 200) + "..." : value;
                    Log.Debug("[Task {TaskId}] Function argument - {Key}: {Value}", taskId, arg.Key, truncated);
                }
            }
        };
        ...
        kernel.FunctionInvoked += (sender, e) =>
        {
            ...
            var truncated = resultStr != null && resultStr.Length > 200 ? resultStr.Substring(0, 200) + "..." : resultStr;
            ...
            Log.Debug("[Task {TaskId}] Function result: {Result}", taskId, truncated);
        };
    ...
    {
        Log.Debug("[Task {TaskId}] Adding tools to kernel: {Tools}", taskId, tools);

        if (tools.HasFlag(AgentTools.All))
        {
            kernelBuilder.Plugins.AddFromType<BashTool>();
            kernelBuilder.Plugins.AddFromType<BashOutputTool>();
            kernelBuilder.Plugins.AddFromType<KillBashTool>();
            ...
            kernelBuilder.Plugins.AddFromType<MultiEditTool>();
            kernelBuilder.Plugins.AddFromType<GlobTool>();
            if (!string.IsNullOrEmpty(OpenAIOptions.TAVILY_API_KEY))
            {
                kernelBuilder.Plugins.AddFromType<WebTool>();
            }

            Log.Information("[Task {TaskId}] Added all tools to kernel", taskId);
            return;
        }
    ...
    private Kernel CreateKernel()
    {
        ...
        // Add all tools for backward compatibility
        kernelBuilder.Plugins.AddFromType<BashTool>();
        kernelBuilder.Plugins.AddFromType<BashOutputTool>();
        kernelBuilder.Plugins.AddFromType<KillBashTool>();
        kernelBuilder.Plugins.AddFromType<ReadTool>();
