...
public class TaskTool
{
        ...
        string prompt)
    {
        ...
        try
        {
            ...
            {
                Log.Information("[Task {TaskId}] Task completed with result length: {Length}", taskId,
                    result?.Length ?? 0);
            ...
            var agentTask = Task.Run(async () =>
            {
                ...
                Log.Information("[Task {TaskId}] Agent execution completed, total output length: {Length}", taskId,
                    sb.Length);
            });
    ...
    private static Kernel CreateKernelForAgent(Func<string, Task> completeTaskFunc, string taskId)
    {
        ...
        {
            Log.Information("[Task {TaskId}] Invoking function: {PluginName}.{FunctionName}", taskId,
        ...
        kernel.FunctionInvoked += (sender, e) =>
        {
            ...
            var truncated = resultStr != null && resultStr.Length > 200
